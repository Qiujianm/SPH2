    ...
    while true; do
        ...
        case $choice in
            ...
            6)
                ...
                ;;
            7) bash ./monitor.sh ;;
            8) bash ./config.sh whitelist ;;
            9) protection_management ;;
            10) bash ./config.sh uninstall ;;
            0) exit 0 ;;
            *)
                printf "%b无效选择%b\n" "${RED}" "${NC}"
                sleep 1
...
EOF
    ...
    case "$create_service" in
        ...
        2)
            ...
            local total_ports=${#ports_to_create[@]}
            ...
            for port in "${ports_to_create[@]}"; do
                (
                    if systemctl start "hysteria-server@${port}.service" >/dev/null 2>&1; then
                        echo -e "${GREEN}✓ 已启动新增实例：${domain}_${port}${NC}"
                        ((started_count++))
                    else
                        echo -e "${RED}✗ 启动失败：${domain}_${port}${NC}"
                        ((failed_count++))
                    fi
                ) &
        ...
        4)
            ...
            # 创建快速启动脚本
            local fast_start_script="/tmp/hysteria_fast_start_$$.sh"
...
for i in "${!ports_to_create[@]}"; do
    ...
    config="${configs_to_create[\$i]}"
    
    # 检查是否已有进程在运行
    if ! pgrep -f "hysteria.*server.*-c.*\$config" >/dev/null; then
        # 后台启动hysteria进程
        nohup \$HYSTERIA_BIN server -c "\$config" >/dev/null 2>&1 &
        echo -e "\033[32m✓ 已启动新增实例：\${DOMAIN}_\${port}\033[0m"
        ((started_count++))
    else
        echo -e "\033[33m⚠ 实例 \${DOMAIN}_\${port} 已在运行\033[0m"
    fi
...
CLIENTEOF
    ...
    # 创建并赋权 monitor.sh (流量监控)
    cat > /root/hysteria/monitor.sh << 'MONITOREOF'
#!/bin/bash
...
setup_traffic_collection() {
    ...
    # 创建数据收集脚本
    cat > /usr/local/bin/hysteria-traffic-collector.sh << 'EOFCOLLECTOR'
#!/bin/bash
TRAFFIC_DATA_DIR="/var/log/hysteria/traffic"
mkdir -p "$TRAFFIC_DATA_DIR"
...
EOFCOLLECTOR
    ...
    # 创建 cron 任务（每小时执行一次）
    cat > /etc/cron.d/hysteria-traffic << 'EOFCRON'
# 每小时收集一次流量数据
...
main_menu
MONITOREOF
    chmod +x /root/hysteria/monitor.sh
